...
let ai;
try {
    // API Key được tích hợp trực tiếp
    ai = new GoogleGenAI({ apiKey: "AIzaSyC1DiQKS7prwii1ev3taOpg3bh7yMhXPPU" });
} catch (e) {
...
5.  Luôn trả về một đối tượng JSON duy nhất, hợp lệ theo đúng schema đã định nghĩa.`;
    ...
    try {
        ...
        throw new Error("Không thể phân tích hình ảnh CV. Vui lòng thử lại với hình ảnh rõ nét hơn hoặc kiểm tra cấu hình AI.");
    }
}
...
function renderCVEditor() {
  ...
  DOMElements.cvEditor.innerHTML = `
      <div class="space-y-6">
        ...
        <div class="space-y-4 border border-slate-600 rounded-lg p-4 glass">
        ...
        <div class="space-y-4 border border-slate-600 rounded-lg p-4 glass">
            ...
            <button id="add-experience-btn" class="text-sm font-medium text-blue-400 hover:text-blue-300 transition-colors">+ Thêm kinh nghiệm</button>
        </div>
    ...
    DOMElements.analysisResultContainer.innerHTML = `
        <div class="space-y-8">
            <div class="flex justify-between items-center">
                <h2 class="text-3xl font-black bg-gradient-to-r from-blue-400 via-purple-400 to-green-400 bg-clip-text text-transparent flex items-center gap-3"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-8 h-8 text-blue-400 animate-glow"><path d="M12 3L9.5 8.5L4 11L9.5 13.5L12 19L14.5 13.5L20 11L14.5 8.5L12 3Z"></path><path d="M5 3v4"></path><path d="M19 3v4"></path><path d="M3 5h4"></path><path d="M17 5h4"></path><path d="M5 21v-4"></path><path d="M19 21v-4"></path><path d="M3 19h4"></path><path d="M17 19h4"></path></svg>KẾT QUẢ PHÂN TÍCH TỪ AI</h2>
                <button id="apply-suggestions-btn" class="btn-primary text-white font-bold py-3 px-6 rounded-lg hover-glow shadow-lg">ÁP DỤNG ĐỀ XUẤT</button>
            </div>
            
            <div class="space-y-6">
                <h3 class="font-bold text-xl text-blue-400 border-b border-blue-400/30 pb-2">PHÂN TÍCH TỔNG QUAN</h3>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="glass border border-green-500/30 p-6 rounded-xl">
            ...
            <div class="space-y-6">
                <h3 class="font-bold text-xl text-blue-400 border-b border-blue-400/30 pb-2">ĐỀ XUẤT BỔ SUNG</h3>
                 <div class="p-6 glass border border-blue-500/30 rounded-xl">
             ...
             <div class="space-y-6">
                ...
                <div class="space-y-4">
                    ${detailedCorrections.map(item => `
                        <div class="p-4 glass border border-slate-600 rounded-xl">
